...
<div class="container px-2.5 md:px-4 mt-4 dark:bg-gray-900">
    ...
    <h2 class="mb-4 dark:text-white">{{ $categoryName ?? 'Kerukunan' }}</h2>
            ...
            <a href="{{ route('artikel.view', $articles->first()->id) }}">
                <div class="relative h-[20rem] md:h-[28rem] rounded-xl overflow-hidden">
                    ...
                    <div class="absolute bottom-0 left-0 right-0 p-4 md:p-6 bg-gradient-to-t from-black/80 to-transparent">
                        ...
                        <h2 class="text-white text-xl md:text-2xl font-bold">{{ $articles->first()->title }}</h2>
                    </div>
                </div>
            </a>
        </div>
                    ...
                    @if(isset($popularArticlesAll) && $popularArticlesAll->isNotEmpty())
                        @foreach($popularArticlesAll as $index => $article)
                            <a href="{{ route('artikel.view', $article->id) }}" class="flex items-center gap-4 p-4 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                                <span class="text-xl md:text-2xl font-bold text-blue-900 dark:text-blue-400">{{ $index + 1 }}</span>
                                <p class="font-medium text-sm md:text-base text-gray-800 dark:text-gray-200">{{ $article->title }}</p>
                            </a>
                        @endforeach
                    @else
                        @php
    ...
    <div class="grid grid-cols-1 md:grid-cols-12 gap-6">
        ...
        <div class="col-span-1 md:col-span-8">
            <div class="grid grid-cols-1 gap-6">
                ...
                <a href="{{ route('artikel.view', $article->id) }}" class="flex flex-col md:flex-row gap-4 items-start">
                    ...
                    @if(isset($editorPicks) && $editorPicks->isNotEmpty())
                        ...
                        <div class="group">
                            <a href="{{ route('artikel.view', $article->id) }}" class="flex items-start gap-3">
                                <img src="{{ $article->image ? url('images/' . $article->image) : asset('images/berita6.jpg') }}" alt="{{ $article->title }}" class="w-16 md:w-20 h-16 md:h-20 object-cover rounded-lg">
                                <div>
    ...
    <div class="bg-white dark:bg-gray-800 p-8 rounded-xl shadow-sm text-center">
        ...
        <p class="text-gray-600 dark:text-gray-300">Saat ini belum ada artikel tersedia dalam kategori Kerukunan.</p>
    </div>
    @endif
</div>
@endsection
