...
<div class="container px-2.5 md:px-4 mt-4 dark:bg-gray-900">
    ...
    <h2 class="mb-4 dark:text-white">{{ $categoryName ?? 'Khutbah' }}</h2>
                    ...
                    @if(isset($editorPicks) && $editorPicks->isNotEmpty())
                        ...
                        <div class="group">
                            <a href="{{ route('artikel.view', $article->id) }}" class="flex items-start gap-3">
                                <img src="{{ $article->image ? url('images/' . $article->image) : asset('images/berita6.jpg') }}" alt="{{ $article->title }}" class="w-20 h-20 object-cover rounded-lg">
                                <div>
    ...
    <div class="bg-white dark:bg-gray-800 p-8 rounded-xl shadow-sm text-center">
        ...
        <p class="text-gray-600 dark:text-gray-300">Saat ini belum ada artikel tersedia dalam kategori Khutbah.</p>
    </div>
    @endif
</div>
@endsection
