...
<div class="container px-2.5 sm:px-4 mt-4 dark:bg-gray-900">
    ...
    <h2 class="mb-4 dark:text-white">{{ $categoryName ?? 'Pendidikan' }}</h2>
                    ...
                    @if(isset($popularArticlesAll) && $popularArticlesAll->isNotEmpty())
                        @foreach($popularArticlesAll as $index => $article)
                            <a href="{{ route('artikel.view', $article->id) }}" class="flex items-center gap-4 p-4 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                                <span class="text-2xl font-bold text-blue-900 dark:text-blue-400">{{ $index + 1 }}</span>
    ...
    <div class="bg-white dark:bg-gray-800 p-8 rounded-xl shadow-sm text-center">
        ...
        <p class="text-gray-600 dark:text-gray-300">Saat ini belum ada artikel tersedia dalam kategori Pendidikan.</p>
    </div>
    @endif
</div>
@endsection
