    ...
    <div class="p-6">
        ...
        <form action="{{ route('articles.update', $article->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                ...
                <select id="category" name="category" required
                    ...
                    @foreach ($categories as $category)
                        <option value="{{ $category }}" {{ old('category', $article->category) == $category ? 'selected' : '' }}>{{ ucfirst($category) }}</option>
                    @endforeach
                </select>
            </div>
                ...
                @if ($article->image)
                    <div class="mb-4">
                        <div class="flex items-center">
                            <img src="{{ url('images/' . $article->image) }}" class="h-32 w-48 object-cover rounded border" alt="Gambar Artikel Saat Ini">
                            <div class="ml-4">
                                <p class="text-sm text-gray-500">Gambar saat ini</p>
                                <div class="mt-2 text-xs text-gray-500">
                                    <p>Unggah gambar baru untuk mengganti gambar saat ini</p>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div>
                ...
                <textarea id="description" name="description" class="summernote">{{ old('description', $article->description) }}</textarea>
            </div>
